...
5. To install the GUI and NWB dependencies, run `python -m pip install suite2p[all]`. If you're on a zsh server, you may need to use `' '` around the suite2p[all] call: `python -m pip install 'suite2p[all]'`.
6. Instead, you can install the minimal version of suite2p, run `python -m pip install suite2p`.  
