...
from numba import vectorize,float32,int32,int16,jit,njit,guvectorize,prange
...
def phasecorr(data, refAndMasks, ops):
    ...
    for n in range(nb):
        ...
        #Xt = apply_masks(Xt, maskMul[n], maskOffset[n])
        #Xt *= maskMul[n]
        #Xt += maskOffset[n]
        #print('xymask %2.2f'%toc(t0))
        for t in range(nimg):
            Xt = X[t][np.ix_(yind,xind)]
            Y[t] = register.fft2(Xt*maskMul[n] + maskOffset[n])
        #print('fft1 %2.2f'%toc(t0))
        Y = register.apply_dotnorm(Y, cfRefImg[n])
        #print('pc %2.2f'%toc(t0))
        for t in range(nimg):
