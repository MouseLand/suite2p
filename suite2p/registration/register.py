...
def compute_crop(xoff: int, yoff: int, corrXY, th_badframes, badframes,
                 maxregshift, Ly: int, Lx: int):
...
def compute_reference(frames, ops=default_ops()):
    ...
    for iter in range(0, niter):
        ...
        refImg = rigid.shift_frame(frame=refImg,
...
def compute_reference_masks(refImg, ops=default_ops()):
    ...
    if isinstance(refImg, list):
        ...
        blocks = []
        if ops.get("nonrigid"):
            blocks = nonrigid.make_blocks(Ly=Ly, Lx=Lx,
...
def register_frames(refAndMasks, frames, rmin=-np.inf, rmax=np.inf,
                    bidiphase=0, ops=default_ops(), nZ=1):
    ...
    if nZ > 1:
        ...
        for z in range(nZ):
            ops["nonrigid"] = False
            outputs = register_frames(refAndMasks[z], frames.copy(),
                                      rmin=rmin[z], rmax=rmax[z],
            ...
            for i, (output_best,
                    output) in enumerate(zip(outputs_best, outputs[:-4])):
                output_best[ibest] = output[ibest]
        if run_nonrigid:
            ops["nonrigid"] = True
            nfr = frames.shape[0]
            for i, z in enumerate(zpos_best):
                outputs = register_frames(refAndMasks[z], frames[[i]],
                                          rmin=rmin[z], rmax=rmax[z],
                                          bidiphase=bidiphase, ops=ops, nZ=1)
        ...
        if ops["1Preg"]:
            ...
            fsmooth = utils.spatial_high_pass(fsmooth,
                                              int(ops["spatial_hp_reg"]))

        # rigid registration
        ymax, xmax, cmax = rigid.phasecorr(
            data=rigid.apply_masks(
                data=np.clip(fsmooth, rmin, rmax) if rmin > -np.inf else
                fsmooth, maskMul=maskMul, maskOffset=maskOffset),
            cfRefImg=cfRefImg,
            maxregshift=ops["maxregshift"],
            smooth_sigma_time=ops["smooth_sigma_time"],
        )
        ...
        if ops["nonrigid"]:
            ...
            ymax1, xmax1, cmax1 = nonrigid.phasecorr(
                data=np.clip(fsmooth, rmin, rmax)
                if rmin > -np.inf else fsmooth,
...
def shift_frames(frames, yoff, xoff, yoff1, xoff1, blocks=None,
                 ops=default_ops()):
    ...
    if ops["nonrigid"]:
        frames = nonrigid.transform_data(
            frames, yblock=blocks[0], xblock=blocks[1], nblocks=blocks[2],
...
def compute_reference_and_register_frames(f_align_in, f_align_out=None,
                                          refImg=None, ops=default_ops()):
    ...
    if refImg is None:
        # grab frames
        frames = f_align_in[np.linspace(
            0, n_frames, 1 + np.minimum(ops["nimg_init"], n_frames),
            dtype=int)[:-1]]
        # compute bidiphase shift
        if ops["do_bidiphase"] and ops[
                "bidiphase"] == 0 and not ops["bidi_corrected"]:
            bidiphase = bidi.compute(frames)
            print("NOTE: estimated bidiphase offset from data: %d pixels" %
                  bidiphase)
    ...
    for k in np.arange(0, n_frames, batch_size):
        ...
        frames, ymax, xmax, cmax, ymax1, xmax1, cmax1, zest = register_frames(
            refAndMasks, frames, rmin=rmin, rmax=rmax, bidiphase=bidiphase,
            ops=ops, nZ=nZ)
...
def shift_frames_and_write(f_alt_in, f_alt_out=None, yoff=None, xoff=None,
                           yoff1=None, xoff1=None, ops=default_ops()):
    ...
    for k in np.arange(0, n_frames, batch_size):
        ...
        frames = shift_frames(frames, yoffk, xoffk, yoff1k, xoff1k, blocks,
                              ops)
        mean_img += frames.sum(axis=0) / n_frames
        ...
        if (ops["reg_tif_chan2"] if ops["functional_chan"]
                == ops["align_by_chan"] else ops["reg_tif"]):
            fname = io.generate_tiff_filename(
                functional_chan=ops["functional_chan"],
                align_by_chan=ops["align_by_chan"], save_path=ops["save_path"],
                k=k, ichan=False)
...
def registration_wrapper(f_reg, f_raw=None, f_reg_chan2=None, f_raw_chan2=None,
    ...
    outputs = compute_reference_and_register_frames(f_align_in,
    ...
    if nchannels > 1:
        mean_img_alt = shift_frames_and_write(f_alt_in, f_alt_out, yoff, xoff,
                                              yoff1, xoff1, ops)
    else:
        mean_img_alt = None
    ...
    if "data_path" in ops and len(ops["data_path"]) > 0:
        badfrfile = path.abspath(
            path.join(ops["data_path"][0], "bad_frames.npy"))
