...
def run_plane(ops, ops_path=None, stat=None):
    ...
    with io.BinaryRWFile(
        Ly=Ly, Lx=Lx, filename=raw_file if raw else reg_file
    ) as f_raw, io.BinaryRWFile(
        Ly=Ly, Lx=Lx, filename=reg_file
    ) as f_reg, io.BinaryRWFile(
        Ly=Ly, Lx=Lx, filename=raw_file_chan2 if raw else reg_file_chan2
    ) as f_raw_chan2, io.BinaryRWFile(
        Ly=Ly, Lx=Lx, filename=reg_file_chan2
    ) as f_reg_chan2:
        if not raw:
            f_raw.close()
            f_raw = None
...
def run_s2p(ops={}, db={}, server={}):
    ...
    # if not set up files and copy tiffs/h5py to binary
        ...
        convert_funs = {
            ...
            "haus": lambda ops: haussio.load_haussio(ops["data_path"][0]).tosuite2p(
                ops.copy()
            ),
