...
def extract_wrapper(settings):
    for plane in range(settings[0]['nplanes']):
        curr_op = settings[plane]
        ...
        np.save(plane_dir.joinpath('settings.npy'), curr_op)
...
def run_preprocess(f: np.ndarray, test_settings):
    baseline_vals = ['maximin', 'constant', 'constant_prctile']
    for bv in baseline_vals:
        pre_f = extraction.preprocess(
            F=f,
            baseline=bv,
            win_baseline=test_settings['win_baseline'],
            sig_baseline=test_settings['sig_baseline'],
            fs=test_settings['fs'],
            prctile_baseline=test_settings['prctile_baseline']
        )
        test_f = np.load(test_settings['data_path'][0].parent.joinpath('test_outputs/extraction/{}_f.npy'.format(bv)))
        yield np.allclose(pre_f, test_f, rtol=1e-4, atol=5e-2)


def test_pre_process_baseline(test_settings):
    f = np.load(test_settings['data_path'][0].parent.joinpath('test_outputs/1plane1chan1500/suite2p/plane0/F.npy'))
    assert all(run_preprocess(f, test_settings))
...
def test_extraction_output_1plane1chan(test_settings):
    ...
    for i in range(nplanes):
        assert all(utils.compare_list_of_outputs(
            outputs_to_check,
            utils.get_list_of_data(outputs_to_check, Path(settings['data_path'][0]).parent.joinpath(f"test_outputs/extraction/1plane1chan/plane0")),
            utils.get_list_of_data(outputs_to_check, Path(settings['save_path0']).joinpath(f"suite2p/plane0")),
        ))
...
def test_extraction_output_2plane2chan(test_settings):
    ...
    for i in range(len(settings)):
        assert all(utils.compare_list_of_outputs(
            outputs_to_check,
            utils.get_list_of_data(outputs_to_check, Path(settings[i]['data_path'][0].parent.joinpath(f"test_outputs/extraction/2plane2chan/plane{i}"))),
            utils.get_list_of_data(outputs_to_check, Path(settings[i]['save_path0']).joinpath(f"suite2p/plane{i}")),
        ))