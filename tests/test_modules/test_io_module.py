...
class TestSuite2pIoModule:
    ...
    """
    def test_tiff_reconstruction_from_binary_file(self, default_ops, data_dir):
        """
        Tests to see if tif generated by tiff_to_binary and write_tiff matches test tif.
        """
        op = io.tiff_to_binary(default_ops)[0]
        output_data = get_binary_file_data(default_ops['save_path0'], op)
        ...
        prior_data = imread(
            str(Path(data_dir).joinpath('1plane1chan', 'suite2p', 'test_write_tiff.tif'))
        )
        assert np.array_equal(reconstructed_tiff_data, prior_data)

    def test_h5_to_binary_nonnegative_output(self, default_ops, data_dir):
        """
        Tests if the binary file produced by h5_to_binary contains nonnegative data.
        """
        default_ops['h5py'] = Path(data_dir).joinpath('input.h5')
